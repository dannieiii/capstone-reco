    ...
    <div class="pdf-preview-panel" ref="previewPanelRef">
      <header class="preview-header">
        <div>
          ...
          <h2>Customer Export Summary</h2>
          <p class="preview-subtitle">Review the customers currently visible in your table before printing or saving.</p>
        </div>
        <button
          class="close-btn"
          type="button"
      ...
      <section class="preview-meta">
        ...
        <div>
          <p class="meta-label">Customers in View</p>
          <p class="meta-value">{{ customers.length }}</p>
        </div>
        <div>
          <p class="meta-label">Verified Customers</p>
          <p class="meta-value">{{ verifiedCount }}</p>
        </div>
        <div>
          <p class="meta-label">New This Month</p>
          <p class="meta-value">{{ newThisMonthCount }}</p>
        </div>
      </section>
      ...
      <section class="preview-table-wrapper">
        <table class="preview-table">
          ...
          <tbody>
            <tr v-for="customer in customers" :key="customer.id || customer.userId">
              ...
              <td>
                <span :class="['status-chip', customer.isVerified ? 'status-verified' : 'status-pending']">
                  {{ customer.isVerified ? 'Verified' : 'Pending' }}
                </span>
              </td>
            </tr>
            <tr v-if="customers.length === 0">
              <td colspan="6" class="empty-row">No customers available for export.</td>
            </tr>
          </tbody>
        </table>
      </section>
      ...
      <footer class="preview-footer" data-html2canvas-ignore="true">
        ...
        <p class="preview-hint">Need different rows? Adjust filters or pagination in the Customers table, then reopen this preview.</p>
      </footer>
    </div>
  </div>
</template>
...
const verifiedCount = computed(() => props.customers.filter(c => c.isVerified).length);
const newThisMonthCount = computed(() => {
  const oneMonthAgo = new Date();
  oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);
  return props.customers.filter(c => {
    const created = c.lastUpdated || c.createdAt;
    return created ? new Date(created) > oneMonthAgo : false;
  }).length;
});
...
const formatName = (customer) => {
  ...
  return name || customer.username || 'N/A';
};

const formatAddress = (customer) => {
  return customer.address || customer.fullAddress || 'N/A';
};
...
const formatPrintableRows = () => {
  return props.customers.map(customer => {
    return `
      <tr>
        <td>${escapeHtml(formatName(customer))}</td>
        <td>${escapeHtml(customer.username || 'N/A')}</td>
        <td>${escapeHtml(customer.email || 'N/A')}</td>
        <td>${escapeHtml(customer.contactNumber || 'N/A')}</td>
        <td>${escapeHtml(formatAddress(customer))}</td>
        <td>${customer.isVerified ? 'Verified' : 'Pending'}</td>
      </tr>
    `;
  }).join('');
};
...
const buildPrintableHtml = () => {
  ...
  <html>
    <head>
      <meta charset="utf-8" />
      <title>Customer Export Preview</title>
      <style>
    ...
    <body>
      <h1>Customer Export Preview</h1>
      <p class="subtitle">Generated on ${formattedGeneratedAt.value} • ${props.customers.length} customers</p>
      <div class="meta">
        <div class="meta-item"><strong>Verified:</strong> ${verifiedCount.value}</div>
        <div class="meta-item"><strong>New This Month:</strong> ${newThisMonthCount.value}</div>
      </div>
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          ${formatPrintableRows() || '<tr><td colspan="6">No customers available.</td></tr>'}
        </tbody>
      </table>
    </body>
  </html>`;
...
const handleSavePdf = async () => {
  ...
  try {
    ...
    pdf.save(`customer-export-${new Date().toISOString().slice(0, 10)}.pdf`);
  } catch (error) {
