  ...
  <div v-if="modelValue" class="pdf-preview-overlay">
    <div class="pdf-preview-panel" ref="panelRef">
      ...
      <section class="chart-preview">
        <img v-if="chartSrc" :src="chartSrc" alt="Price analysis chart" />
      ...
      <footer class="preview-footer" data-html2canvas-ignore="true">
        ...
        <p class="preview-hint">Only the current chart snapshot is included. Switch chart views or filters, then reopen this preview for a new snapshot.</p>
      </footer>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  title: {
    type: String,
    default: 'Price Analysis - Oriental Mindoro'
  },
  subtitle: {
    type: String,
    default: ''
...
const buildPrintableHtml = () => {
  ...
  const chartBlock = props.chartSrc
    ? `<img src="${props.chartSrc}" alt="Price analysis chart" />`
...
const handleSavePdf = async () => {
  ...
  try {
    ...
    pdf.save(`price-analysis-${new Date().toISOString().slice(0, 10)}.pdf`);
  } catch (error) {
