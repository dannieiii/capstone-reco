        ...
        <div class="form-grid">
          ...
          <div class="form-section">
            ...
            <div class="form-group">
              ...
              <select id="category" v-model="product.category" required @change="onCategoryChange">
                ...
                <div class="form-group">
                  ...
                  <div class="product-select-container">
                    <select 
                      id="productName" 
                      v-model="selectedProductFromTable" 
                      required
                    ...
                    >
                      <option value="" disabled>
                        {{ !product.category ? 'Select category first' : 'Select product name' }}
                      </option>
                      <option 
                        v-for="productItem in filteredProductTable" 
                        :key="productItem.id" 
                        :value="productItem"
                      >
                    ...
                    <span v-if="selectedProductFromTable" class="price-range-display">
                      ₱{{ selectedProductFromTable.minPrice }}-{{ selectedProductFromTable.maxPrice }} {{ selectedProductFromTable.unit }}
                    </span>
                  </div>
                  
                      <div v-if="selectedProductFromTable" class="selected-product-info">
                        <small class="text-muted">
                          <strong>Selected:</strong> {{ selectedProductFromTable.productName }}
          ...
          <div class="form-section wide">
            ...
            <div v-if="selectedProductFromTable && !useCustomProductName" class="da-price-reference">
              <div class="reference-card">
                <h4>D.A. Reference Price</h4>
                <div class="reference-details">
                  <span class="reference-range">₱{{ selectedProductFromTable.minPrice }} - ₱{{ selectedProductFromTable.maxPrice }}</span>
                  <span class="reference-unit">{{ selectedProductFromTable.unit }}</span>
                ...
                <small class="reference-note">Use this as a guide for competitive pricing</small>
              </div>
            </div>
    ...
    const fetchProduct = async (productId) => {
      try {
        ...
        if (productSnap.exists()) {
          ...
          product.value = {
            ...data,
               variety: data.variety || 'Normal', // Set to Normal if not specified
            id: productSnap.id,
            productId: productSnap.id
          };
          selectedUnits.value = data.availableUnits || [];

          // Check if this product matches any in the product table
          const matchingProduct = productTable.value.find(p =>
            p.productName === data.productName && p.category === data.category
          );
          if (matchingProduct) {
            selectedProductFromTable.value = matchingProduct;
            useCustomProductName.value = false;
          } else {
    ...
    const saveProduct = async () => {
      ...
      try {
        const productData = {
          ...
          // Store reference to D.A. product if selected
          daProductReference: selectedProductFromTable.value ? {
            id: selectedProductFromTable.value.id,
            productName: selectedProductFromTable.value.productName,
            minPrice: selectedProductFromTable.value.minPrice,
            maxPrice: selectedProductFromTable.value.maxPrice,
            unit: selectedProductFromTable.value.unit,
      variety: selectedProductFromTable.value.variety || 'Normal' // Ensure variety is included
          } : null,
...
/* Remove the old price reference table styles */
