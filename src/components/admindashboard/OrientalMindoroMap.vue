    ...
    <div v-if="loading" class="loading-container">
      ...
      >
        ...
        <l-marker
          ...
          :icon="getMarkerIcon(municipality.name)"
        >
          <l-popup>
...
// Leaflet icon setup
const createMarkerIcon = (color = '#3182bd') => {
  return L.divIcon({
...
const zoom = ref(10);
const center = ref([13.0, 121.2]); // Center of Oriental Mindoro
const bounds = ref([
  [12.2, 120.8], // Southwest bounds
  [13.6, 121.6]  // Northeast bounds
]);

// Oriental Mindoro municipalities with real coordinates
...
// Function to get marker icon based on user count
const getMarkerIcon = (municipalityName) => {
  const count = getMunicipalityUsers(municipalityName).length;
  let color = '#f2f2f2'; // No users
  
  if (count <= 5) color = '#eff3ff';
  else if (count <= 15) color = '#bdd7e7';
  else if (count <= 30) color = '#6baed6';
  else if (count <= 50) color = '#3182bd';
  else color = '#08519c';
...
const onMapReady = () => {
  ...
  // Update municipality markers after processing user data
  updateMunicipalityMarkers();
};

// Function to update municipality markers
const updateMunicipalityMarkers = () => {
  municipalityMarkers.value = municipalities.map(municipality => ({
    ...municipality,
    userCount: getMunicipalityUsers(municipality.name).length
  }));
};
...
const processUserData = () => {
  ...
  // Update municipality markers after processing data
  updateMunicipalityMarkers();
};
